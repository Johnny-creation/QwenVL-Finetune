...
def qwen2_5_mixed_modality_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if pixel_values is None and pixel_values_videos is None:
        # Create dummy pixel_values and grid_thw for avoiding deepspeed error.
        dummy_pixel = torch.zeros(784, 1176, device=next(self.visual.parameters()).device)
        dummy_grid = torch.tensor([[1, 28, 28]], device=next(self.visual.parameters()).device)
    
        image_embeds = self.get_image_features(dummy_pixel, dummy_grid)
        # 有些版本视觉塔返回 tuple/list，这里统一取第 0 个张量
        if isinstance(image_embeds, (tuple, list)):
            image_embeds = image_embeds[0]
    
        # Operates as masked_scatter for the image tokens
        # However the values are all zeros so it doesn't affect the embeddings.
        # This avoids DeepSpeed errors when some batches only contain text.
