...
if __name__=='__main__':
    ...
    def mllm_openai(query, images):
        ...
        for image_path in images:
            #full_path = os.path.join(os.path.join(os.path.dirname(os.getcwd()),'model_generation'), image_path)
            #print(full_path)
        ...
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {
                "role": "user",
                "content": [
                    *[{"type": "image", "image": f"data:image/jpeg;base64,{base64_image}"} for
        ...
        inputs.to('cuda:0' if torch.cuda.is_available() else 'cpu')
        print(inputs.input_ids)
        generated_ids = model.generate(**inputs, max_new_tokens=192)
        generated_ids_trimmed = [output_ids[len(input_ids):] for input_ids, output_ids in
    ...
    ### local_index from 0-10. Divide the 110 conversations into 11 parts for testing.
    local_index = 0
    ### scores and reasons save path
    file_save_fold = "../qwen2.5_3b_3b_mark"
    os.makedirs(file_save_fold, exist_ok=True)
    ### model generation answer
    file_get_fold = "../qwen2.5-vl-3b-gene"
    ...
    for benchmark_i in tqdm(range(len(benchmarks))):
        try:
            ...
            try:
                ...
                print(f'{file_get_fold}/{id}.json does not exist')
                continue
            ...
            for j in range(len(questions)):
                try:
                    ...
                    response = mllm_openai(promt, images)
